    ...
    do
        ...
        Library.Dropdown = function(Self, Params)
            ...
            local Items = { } do 
                ...
                }):AddToTheme({BackgroundColor3 = 'Background', ScrollBarImageColor3 = 'Accent'})
                ...
                Items["_Layout"] = Library:Create("UIListLayout", {
                    Name = "\0",
                    Parent = Items["OptionHolder"].Instance,
                    Padding = UDim.new(0, 8),
                    SortOrder = Enum.SortOrder.LayoutOrder
            ...
            function Dropdown:Add(Value)
                local OptionButton = Library:Create("TextButton", {
                    ...
                    Text = Value,
                    AutoButtonColor = false,
                    Size = UDim2.new(1, 0, 0, Library.FontSize or 14),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0
                }):AddToTheme({TextColor3 = 'Inactive Text'})
            ...
            function Dropdown:SetOpen(Bool)
                ...
                if Dropdown.IsOpen then 
                    ...
                    local Scale = Library:GetScreenScale()
                    local maxH = Params.Max or 250
                    local w = RealDropdown.AbsoluteSize.X / Scale
                    OptionHolder.Position = Library:PopupPosition(RealDropdown, OptionHolder, 0)

                    local optionCount = 0
                    for _ in pairs(Dropdown.Options) do optionCount = optionCount + 1 end
                    local textH = Library.FontSize or 14
                    local contentH = 16 + (optionCount * textH) + (math.max(optionCount - 1, 0) * 8)
                    local targetH = math.max(math.min(contentH, maxH), 32)
                    OptionHolder.CanvasSize = UDim2.new(0, 0, 0, contentH)
                    OptionHolder.Size = UDim2.new(0, w, 0, targetH)
